...
ENT.Author			= "Garry Newman, Modified by Exploderguy/DatFancyChicken"
ENT.Information		= "A leathal dodgeball to kill people"
...
function ENT:Initialize()
	...
	if ( SERVER ) then
		...
		if ( self.Owner:Team() == 1 ) then
			self:SetBallColor( Vector( 1, 0.3, 0.3 ) ) -- Red
		else
			self:SetBallColor( Vector( 0.2, 0.3, 1 ) ) -- Blue
		end
...
function ENT:EndTouch( entity )
	if ( entity:IsPlayer() && !entity:HasGodMode() ) then
		if not (entity:Team() == self.Owner:Team()) then
			if not entity:Alive() then return end
			entity:Kill()
			self.Owner:SendLua("LocalPlayer():alertKill()")
			local num = self.Owner:GetNWInt("db_Kills") + 1
			self.Owner:SetNWInt("db_Kills", num)
			net.Start("db_alertKilledBy")
